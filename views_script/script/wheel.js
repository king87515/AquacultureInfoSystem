...
class WeatherWheel {
  ...
  _process() {
    ...
    this._chartData = this._data.map((d) => {
      ...
      cond = (
        cond.startsWith("Rain")
          ? "Rain"
          : cond.startsWith("Snow")
          ? "Snow"
          : cond
      ).replaceAll(" ", "_");
      // console.log("condition2:",cond);
      const datum = {
  ...
  _initScales(lows, highs, precs, humis) {
    ...
    this._b = d3.scaleLinear().domain(d3.extent(precs)).range([0, r.bubble]);
    ...
    this._hc = d3.scaleLinear().domain(hext).range(["#fefefe", "#dedede"]);
  }
  ...
  _initGradients() {
    ...
    this._g
      ...
      .call((g) =>
        g.append("stop").attr("stop-color", c.high).attr("offset", 0)
      )
      .call((g) =>
        g.append("stop").attr("stop-color", c.mid).attr("offset", 0.5)
      )
      .call((g) =>
        g.append("stop").attr("stop-color", c.low).attr("offset", 1)
      );

    this._condColors.forEach((d) =>
      this._g
        .append("linearGradient")
  ...
  _initDynamicParts() {
    ...
    this._statistics = this._g
      ...
      .attr(
        "transform",
        `translate(${half.width - r.inner - half.width},${
          half.height - r.inner - half.height
        })`
      )
      .call((g) =>
        g
      ...
      .on("click", () => {
        ...
        if (this._yearStat) this._showStatistics(this._chartData, this._year);
        else
          this._showStatistics(
            this._getMonthData(this._currMonth),
            this._months[this._currMonth]
          );
      });

    const t = this._texts,
      c = this._colors;
    this._dailyInfo = this._g
      .append("g")
      ...
      .call((g) =>
        g
          .append("g")
          .attr("transform", `translate(${-r.inner / 4},${-r.inner / 4})`) // .attr("transform", `translate(0,${-r.inner / 4})`)
          .call(
            (g) =>
              (t.date = g.append("text").attr("fill", "#aaa").text("Date: "))
          )
          .call(
            (g) =>
              (t.high = g
  ...
  _renderMarks() {
    ...
    addMark(
      "hottest",
      c.high,
      this._y(hottest.high),
      this._y(hottest.high) + 10,
      hottest,
      this._y(hottest.high),
      -2,
      `${hottest.dateStr} - ${hottest.high}${this._tempUnit}`,
      true
    );
    addMark(
      "coldest",
      c.low,
      this._y(coldest.low) - 10,
      this._y(coldest.low),
      coldest,
      this._y(coldest.low),
      "1em",
      `${coldest.dateStr} - ${coldest.low}${this._tempUnit}`,
      true
    );
    addMark(
      "rainiest",
      ...
      r.max - r.label - 10,
      0,
      `${rainiest.dateStr} - ${rainiest.precipitation}`
    ); //+ ${this._precUnit}
  }
  ...
  _renderOverlay() {
    ...
    this._g
      ...
      .on("mouseenter", (e, d) => {
        ...
        t.high.text(`最高水溫: ${d.high}${this._tempUnit}`);
        t.low.text(`最低水溫: ${d.low}${this._tempUnit}`);
        t.avg.text(`平均水溫: ${d.avg}${this._tempUnit}`);
        t.prec.text(`魚隻數量: ${d.precipitation}`); //+ ${this._precUnit}
        t.humidity.text(`鹽度: ${d.humidity}‰`); //%
  ...
  _renderMonthLabels() {
    ...
    this._g
      ...
      .on("mouseover", (e, d) => {
        ...
        this._showStatistics(
          this._getMonthData(d.index),
          this._months[d.index]
        );
      ...
      .join(
        (enter) => {
          return enter
            .append("g")
            .call((g) =>
              g.append("circle").attr("opacity", 0.65).attr("stroke-width", 0.5)
            )
            .call((g) =>
              g
                .append("text")
  ...
  _pack(data) {
    ...
    return d3.pack().size([w, w]).padding(1)(
      d3.hierarchy({ children: counts }).sum((d) => d.count)
    );
  }
